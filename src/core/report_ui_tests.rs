...
mod tests {
    ...
    use crate::core::action_matrix::{AssetActionDecision, AssetAction};
    use crate::core::asset_state::AssetState;
    use crate::core::runtime_mode::ExecutionMode;
    use chrono::Utc;
    use std::collections::HashMap;

    fn mock_asset(symbol: &str, action: AssetAction, state: AssetState, changed: bool) -> AssetActionDecision {
        AssetActionDecision {
            ...
            prev_action: if changed { Some(AssetAction::HOLD) } else { None },
    ...
    fn test_telegram_v3_ui_defensive() {
        let assets = vec![
            mock_asset("SPY", AssetAction::AVOID, AssetState::DEFEND, false),
        ];
        let packet = DecisionPacket {
            date: Utc::now().date_naive(),
            market_features: Default::default(),
    ...
    fn test_telegram_v3_ui_data_error() {
        let assets = vec![
            mock_asset("AAPL", AssetAction::HOLD, AssetState::OPTIMAL, false),
        ];
        let packet = DecisionPacket {
            date: Utc::now().date_naive(),
            market_features: Default::default(),
