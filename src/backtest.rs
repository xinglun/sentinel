...
pub async fn run_backtest(config: &AppConfig, from_date_str: &str, to_date_str: &str) -> Result<()> {
    let from_date = NaiveDate::parse_from_str(from_date_str, "%Y-%m-%d")?;
    let to_date = NaiveDate::parse_from_str(to_date_str, "%Y-%m-%d")?;
    
    let from_dt = OffsetDateTime::from_unix_timestamp(from_date.and_hms_opt(0,0,0).unwrap().and_utc().timestamp()).ok();
    let to_dt = OffsetDateTime::from_unix_timestamp(to_date.and_hms_opt(23,59,59).unwrap().and_utc().timestamp()).ok();
    ...
    for (i, current_date) in simulation_dates.iter().enumerate() {
        ...
        for entry in config.watchlist.iter().filter(|w| w.enable) {
            if let Some(hist) = histories.get(&entry.symbol) {
                ...
                let sliced_bars: Vec<DailyBar> = hist.bars.iter().filter(|b| b.date <= *current_date).cloned().collect();
                if sliced_bars.is_empty() { continue; }
                ...
                if let Some((_, prev_state)) = prev_states.get(&entry.symbol) {
                    if *prev_state != snap.state_code && snap.state_code != "UNKNOWN" && prev_state != "UNKNOWN" {
                        let key = (prev_state.clone(), snap.state_code.clone());
                        *transition_matrix.entry(key).or_insert(0) += 1;
                    }
                }
                prev_states.insert(entry.symbol.clone(), (*current_date, snap.state_code.clone()));
                
                // Record Reliability Signal
                if snap.state_code != "UNKNOWN" {
                    let bucket = if snap.confidence_score >= 90 { "90-100" }
                                 else if snap.confidence_score >= 80 { "80-90" }
                                 else if snap.confidence_score >= 70 { "70-80" }
                                 else if snap.confidence_score >= 60 { "60-70" }
                                 else if snap.confidence_score >= 50 { "50-60" }
                                 else { "<50" };
                    ...
                    if i + 20 < simulation_dates.len() {
                        ...
                        if let (Some(curr), Some(fut)) = (curr_bar, fut_bar) {
                            ...
                            let is_bear_state = snap.state_code == "DEFEND" || snap.state_code == "fear_downtrend";
                            ...
                            if is_bear_state {
                                if fwd_return < 0.0 { is_correct_for_fwd = true; }
                            } else {
                                if fwd_return > 0.0 { is_correct_for_fwd = true; }
                            }
        ...
        if let Some(curr_idx) = simulation_dates.iter().position(|r| r == date) {
            let get_ret_val = |offset: usize| -> Option<f64> {
                if curr_idx + offset < simulation_dates.len() {
                    ...
                    if let (Some(&c), Some(&f)) = (index_prices.get(date), index_prices.get(&f_date)) {
                        Some((f - c) / c * 100.0)
                    } else { None }
                } else { None }
            };
            ...
            if *energy >= 2.0 {
                pe_csv.push_str(&format!("{},{:.2},{},{},{}\n", date, energy, 
                    r5.map(|v| format!("{:.2}", v)).unwrap_or_default(),
                    r20.map(|v| format!("{:.2}", v)).unwrap_or_default(),
                    r60.map(|v| format!("{:.2}", v)).unwrap_or_default()));
                
                if let Some(v) = r20 { high_pot_20d.push(v); }
                if let Some(v) = r60 { high_pot_60d.push(v); }
            } else if *energy <= 1.0 {
                if let Some(v) = r20 { low_pot_20d.push(v); }
                if let Some(v) = r60 { low_pot_60d.push(v); }
            }
        }
    }
    ...
    let avg_high = if high_conf_len == 0 { 0.0 } else { high_conf_returns.iter().sum::<f64>() / high_conf_len as f64 * 100.0 };
    let avg_low = if low_conf_len == 0 { 0.0 } else { low_conf_returns.iter().sum::<f64>() / low_conf_len as f64 * 100.0 };
    
    let hit_rate_high = if high_conf_len == 0 { 0.0 } else { (high_conf_correct as f64 / high_conf_len as f64) * 100.0 };
    let hit_rate_low = if low_conf_len == 0 { 0.0 } else { (low_conf_correct as f64 / low_conf_len as f64) * 100.0 };
    ...
    summary.push_str(&format!("- **Avg 20d Forward Return**: `{:+.2}%`\n", avg_high));
    ...
    let median = |mut arr: Vec<f64>| -> String {
        if arr.is_empty() { return "N/A".to_string(); }
        arr.sort_by(|a, b| a.partial_cmp(b).unwrap());
        let mid = arr.len() / 2;
        format!("{:+.2}%", arr[mid])
    };
    summary.push_str(&format!("- **High Tension (Potential >= 2.0)**: +20d = `{}`, +60d = `{}`\n", median(high_pot_20d), median(high_pot_60d)));
    summary.push_str(&format!("- **Low Tension (Potential <= 1.0)**: +20d = `{}`, +60d = `{}`\n", median(low_pot_20d), median(low_pot_60d)));
    summary.push_str("\n");
    ...
    for ((from, to), count) in &transition_matrix {
        from_groups.entry(from.clone()).or_default().push((to.clone(), *count));
    }
