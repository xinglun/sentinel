...
impl FutuClient {
    pub async fn connect(addr: &str) -> Result<Self> {
        ...
        let pending_requests: Arc<Mutex<HashMap<u32, oneshot::Sender<(FutuHeader, Vec<u8>)>>>> = Arc::new(Mutex::new(HashMap::new()));
                ...
                Ok((res_header, res_body)) => {
                    if res_header.n_proto_id == 1001 {
                        ...
                        if parsed.ret_type == 0 {
                            ...
                            println!("✅ Successfully authenticated with Moomoo OpenD! Server Ver: {}", s2c.server_ver);
                        } else {
                            anyhow::bail!("InitConnect failed: {:?}", parsed.ret_msg);
                        }
                    } else {
                        anyhow::bail!("Expected InitConnect Response (1001) but got {}", res_header.n_proto_id);
                    }
                },
        ...
        if keep_alive_interval > 0 {
            tokio::spawn(async move {
                use crate::adapters::futu::protocol::generated::keep_alive::{Request as HbRequest, C2s as HbC2s};
                let mut interval = tokio::time::interval(std::time::Duration::from_secs(keep_alive_interval as u64));
                interval.tick().await; 
                loop {
                    interval.tick().await;
                    let req = HbRequest {
                    ...
                    Ok((res_header, res_body)) => {
                        ...
                        if let Some(sender) = pending.remove(&serial) {
                            ...
                            if res_header.n_proto_id == 1004 { // KeepAlive Response
                                ...
                                println!("📩 Unmatched or Push Notification received! ProtoID: {}", res_header.n_proto_id);
                            }
                        }
                    },
                    Err(e) => {
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(10), rx).await {
            Ok(Ok((res_header, res_body))) => {
                if res_header.n_proto_id == proto_id {
                    ...
                    anyhow::bail!("Protocol ID mismatch! Expected {} got {}", proto_id, res_header.n_proto_id)
                }
            },
