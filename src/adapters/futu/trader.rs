...
use crate::trade::trader::{AccountFunds, OrderSide, OrderType, PlaceOrderRequest, PlaceOrderResponse, TradeExecutor};
...
use crate::adapters::futu::protocol::generated::trd_common::{TrdHeader, TrdSide, OrderType as FutuOrderType};
...
impl FutuTrader {
    ...
    fn build_trd_header(&self) -> Result<TrdHeader> {
        let acc_id = self.config.acc_id.ok_or_else(|| anyhow!("Moomoo account ID (FUTU_ACC_ID) is not configured"))?;
...
impl TradeExecutor for FutuTrader {
    async fn unlock_trade(&self) -> Result<()> {
        ...
        if res.ret_type == 0 {
            ...
            Err(anyhow!("Failed to unlock trade: ret_type={}, msg={:?}, err_code={:?}", res.ret_type, res.ret_msg, res.err_code))
        }
    }
    ...
    async fn get_funds(&self) -> Result<AccountFunds> {
        ...
        if res.ret_type != 0 {
            return Err(anyhow!("Failed to get funds: ret_type={}, msg={:?}", res.ret_type, res.ret_msg));
        }

        let s2c = res.s2c.ok_or_else(|| anyhow!("Missing payload in Trd_GetFunds response"))?;
        let funds = s2c.funds.ok_or_else(|| anyhow!("Missing funds details in payload"))?;
    ...
    async fn place_order(&self, order: PlaceOrderRequest) -> Result<PlaceOrderResponse> {
        println!("🚀 Executing PlaceOrder ({}): {:?} qty={} price={:?}", order.symbol, order.side, order.qty, order.price);
        ...
        if res.ret_type != 0 {
            return Err(anyhow!("Failed to place order: ret_type={}, msg={:?}", res.ret_type, res.ret_msg));
        }

        let s2c = res.s2c.ok_or_else(|| anyhow!("Missing payload in Trd_PlaceOrder response"))?;
