...
use stock_sentinel::core::action_matrix::{AssetActionDecision, AssetAction};
...
fn test_kill_switch_behavior_in_execution_gate() {
    ...
    assert_eq!(result.trades.len(), 0, "No trades should be produced when trading is disabled");
    assert_eq!(result.audits.len(), 1);
    assert_eq!(result.audits[0].blocked_by, Some("TradingDisabled".to_string()));
