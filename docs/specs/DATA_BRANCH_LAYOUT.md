...
`reports/` 内のすべての「単一日の資産」は、同じ**報告業務日**の日付キーを使用しなければなりません：

1. `packet.date`（`report_date`）
2. JST のワークフローが生成する報告業務日
3. 市場データの実際の as-of 日とは別の値になり得る

市場データの事実日は `packet.market_features.date` に保持し、正式な
`TradingDaySnapshot` では `market_date` / `as_of_date` に保存します。報告業務日を
