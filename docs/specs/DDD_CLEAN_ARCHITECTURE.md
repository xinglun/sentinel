...
Infrastructure の persisted read boundary は、Domain entity / value object の invariant と category 固有の source type 契約を再検証してから Application へ渡す。category/source type の許可表は `gray_rhino_evidence_source_policy.rs` を SSOT とし、flat persisted record と typed evidence validator は同じ Domain policy を使う。accepted / rejected view は同じ read batch から組み立てる。Interface は scoreable / accepted / rejected の eligibility rule を再実装せず、Application が組み立てた read model と localized rejection label を表示する。
