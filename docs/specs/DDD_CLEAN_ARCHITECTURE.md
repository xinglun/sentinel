...
Infrastructure の persisted read boundary は、Domain entity / value object の invariant を再検証してから Application へ渡す。Interface は scoreable / accepted / rejected の eligibility rule を再実装せず、Application が組み立てた read model を表示する。
