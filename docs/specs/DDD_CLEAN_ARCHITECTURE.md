...
Application は分類閾値、risk lifecycle state 判定、evidence の effective state 集約、互補 evidence category の subject pairing、mitigating evidence から resolved lifecycle event への投影条件などの業務 policy を所有しない。これらは `src/features/<feature>/domain/**` の domain service に置き、Application は port、repository、use case の orchestration に限定する。
Infrastructure の persisted read boundary は、Domain entity / value object の invariant と category 固有の source type 契約を再検証してから Application へ渡す。Valuation Gravity snapshot は要求日、snapshot 日、asset 日、observation-only、分類 tuple、source health / reason の整合性を Domain policy で再検証する。category/source type の許可表は `src/features/research/domain/gray_rhino_evidence_source_policy.rs` を SSOT とし、flat persisted record と typed evidence validator は同じ Domain policy を使う。accepted / rejected view は `src/features/research/infrastructure/gray_rhino_evidence_store.rs` の read batch から組み立て、Application の `GrayRhinoEvidenceReadModel` に渡す。Interface は scoreable / accepted / rejected の eligibility rule を再実装せず、Application が組み立てた read model と localized rejection label を表示する。
...
| Research | research attention、asset thesis、macro gravity、gray rhino monitoring、valuation gravity observation |
...
| Shared | cross-feature primitive、`src/features/shared/interface/i18n.rs`、run status、notification boundary |
...
Strategic Context、Hypothesis Layer、Cognitive Yield、Macro Gravity、Valuation Gravity などの表示専用 layer は、Gate や execution に依存させない。
