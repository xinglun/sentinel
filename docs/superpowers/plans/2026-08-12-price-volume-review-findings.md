...
- [ ] 添加可恢复 DEGRADED 仍输出 PARTIAL/CANDIDATE 的测试。
- [ ] 添加无 Supply Context 时保留价量候选、但不产生供给吸收的测试。
- [ ] 添加事件日期之后没有足够样本时不使用 `POST_*` 的测试。
- [ ] 运行 focused domain tests，确认新增断言先失败。
...
- [ ] 让可恢复 `DEGRADED` 经过指标计算，保留 fail-closed 原因。
- [ ] 为缺失 Supply Context 保留候选观察并限制 `SupplyAbsorption`。
- [ ] 让 event baseline 检查真实事件后样本，不足时退回 `AVAILABLE_HISTORY` 或明确不可用。
- [ ] 运行 focused tests，确认新增测试通过且既有分类不变。
...
- [ ] 补齐 IPO 3/7/15、Lock-up 1/3/5、Earnings 1/3 的独立断言。
- [ ] 补齐 429、volume gap、Supply Context missing、short squeeze/repair、三日生命周期和 boundary 断言。
- [ ] 补齐 report、legacy serde 和事件样本真实性测试。
- [ ] 运行 domain、report、persistence focused tests。
...
- [ ] 勾选并记录本计划的实际执行结果。
- [ ] 更新 Summary 的 scenario coverage、residual risks 和 review focus。
- [ ] 运行 Contract 要求的全部 `make` checks、`make quality` 和独立边界审计。
- [ ] 运行 `make ai-finish TASK=price-volume-review-findings` 归档并确认无 active Work Item。
