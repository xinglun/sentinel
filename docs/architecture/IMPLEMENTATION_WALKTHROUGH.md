...
- **消息通知优化** (Refined)：Telegram 报告现已全面优化。`Age` 标签更新为更具解释性的 `Regime Age`。同时，针对脆弱态（Fragile + IGNITION）引入了“克制化”文案逻辑，在保持动作指引不变的前提下，通过语气打磨（如“适合轻仓跟踪”）平衡进攻性与风险提示。
...
## 10. 资产层连续性与强度记忆 (Asset Layer Continuity)
该模块引入了“资产层相对强度记忆”，解决了判定逻辑在资产维度的“短视”问题：
- **Top Tier Lock**：对于过去 10 天中有 6 天进入强度前 3 名的标的，触发“强者锁定”，确保其状态不低于 `CRUISE`，防止系统因杂讯频繁调出核心领涨资产。
- **Promotion Cap**：对于过去 20 天内有 `DEFEND` 记录或长期处于弱势的资产，其最高状态被限制在 `CRUISE`，防止质地较差的资产因单日大涨而获得 `OPTIMAL` 评级。
- **State Transition Friction**：
    - **降级摩擦**：处于 `OPTIMAL` 的资产需满足连续 2 天失效条件才允许降级。
    - **升级摩擦**：升至 `OPTIMAL` 需连续 3 天满足条件才允许解锁。
- **70/30 Memory-Adjusted Ranking**：资产最终排序不再仅依赖当前离散度，而是结合了 20 日历史表现得分（权重 30%）进行平滑处理，显著提升了 Top Actions 的稳定性。
