...
def feature_acl_violations(path: Path, root: Path, manifest: FeatureAclManifest) -> list[Violation]:
    ...
    if layer == "interface" and rel_path.endswith("_tests.rs"):
        return []
    violations: list[Violation] = []
    text = path.read_text(encoding="utf-8")
    is_acl = layer == "acl"
    ...
    for line_no, import_path in imports_from(path):
        if (
            feature == "radar"
            and layer == "interface"
            and rel_path != "src/features/radar/interface/radar_pipeline_runner.rs"
            and import_path.startswith("crate::config")
        ):
